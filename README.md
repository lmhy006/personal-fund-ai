...
| Phase 4 | **设计阶段（仅 schema）**：工具契约/权限矩阵/错误处理与六类停止条件的实测验证见 `docs/PHASE4_AGENT_SCHEMA.md` 与 `docs/PHASE4_FAILURE_SCENARIOS.md`；先只读 + 用户触发的冻结 `run_production_pipeline`，不接 LLM、不下单、不暴露参数修改/影子转正 |
