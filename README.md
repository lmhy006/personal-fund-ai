...
**主策略口径交叉验证**（`backtest_strategy.py --phase dev --include-delisted --sensitivity`）：Top50 费用后 **11.51%**（现存池 11.84%）、可执行全池 11.93%（现存池 12.43%）、**组合 vs 可执行全池月超额 +0.0108%（NW t=+0.10）**（现存池 −0.0080%、NW −0.08），Top 20/100 分别 11.29%/11.24% → **结论不变：动量 Top50 与可执行全池不可分辨**。档案：`ml/backtest/backtest_report_dev.txt` 为**现存池**版本（`--include-delisted` 本次以 `--no-save` 运行，未覆盖）。
...
**研究目标（2026-09-18 重述）**：特征扩充的目的是**找到费用后稳定的组合增量**（相对全池），不是"让 ML 指标超过动量"；每组以费用后相对全池收益、回撤与分阶段表现评价，IC 仅作辅助；只在 dev 段内按时间滚动取舍。**清盘基金边界**：清盘历史池已部分并入（任务 3），但如上所述**并未消除**幸存者偏差——因此对外表述仍应为"现存 + 部分已清盘池的条件性历史研究"。
